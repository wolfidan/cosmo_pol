...
'ZV':'dBZ','ATT_H':'dBZ','ATT_V':'dBZ'}

VAR_LABELS_SIMUL={'ZH':'Reflectivity','KDP':'Specific diff. phase',\
...
class PyartRadopVProf(PyartRadop):
    def __init__(self, scan):
        if not isinstance(scan['data'],list):
            scan['data']=[[scan['data']]]
        super(PyartRadopVProf,self).__init__('vprof',scan)
    
    def get_field(self,sweep_idx, variable):
        out = super(PyartRadopVProf,self).get_field(sweep_idx, variable)
...
class PyartMXPOL(core.Radar):
    def __init__(self,filename, max_range=np.Inf,min_range=10000):
        ...
        # Create PyART instance
        super(PyartMXPOL,self).__init__(time,rrange,fields,metadata,scan_type,latitude,longitude,altitude,sweep_number,sweep_mode,fixed_angle,\
