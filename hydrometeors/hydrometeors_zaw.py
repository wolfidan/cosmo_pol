...
class MeltingSnow(object):
    ...
    def get_N(self,D):
        ...
        try:
            return operator(self.prop_factor, self.equivalent_rain.get_N(D)) \
                      * self.equivalent_rain.get_V(D) / self.get_V(D)
        except:
            raise
...
#            # Reset vd_interpolator since f_wet has changed
    ...
    def get_m_func(self,T,f):
        def func(D, T, f):
            ...
            return dielectric.dielectric_mixture(frac, [m_water, m_ice, constants.M_AIR])
...
#    luts = load_all_lut('1mom',['R','mS'],9.41,'tmatrix_new')
#    lut = luts['mS'].value_table[0,-1]
#    z = lut[:,0] -lut[:,1] -lut[:,2] +lut[:,3]

    mS = create_hydrometeor('mS','1mom')
    mS.set_psd(np.array([273]),np.array([ 0.001]),np.array([0.99]))
...
#    mS.set_psd(np.array([273.15]),np.array([0.001 *(1-f)]),np.array([1.]))

#    print(np.nansum(r.get_N(D)[0]*z ) +np.nansum(mS.get_N(D)[0]*z))
    plt.plot(D,mS.get_N(D)[0],D,r.get_N(D)[0])


    from cosmo_pol.hydrometeors.hydrometeors import create_hydrometeor

#
#    mS = create_hydrometeor('mG','1mom')
#    mS.set_psd(np.array([ 0.001]),np.array([0.99]))
#
#    r = create_hydrometeor('R','1mom')
#    r.set_psd(np.array([ 0.001]))
##    r = create_hydrometeor('R','1mom')
