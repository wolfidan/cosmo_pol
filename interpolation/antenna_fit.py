...
def optimize_gaussians(x,y,n_gaussians):
    ...
    params = minimize(obj,p0,args=(x,y),bounds=bounds,method='SLSQP')
...
if __name__ == 'main':
    ...
    x_leg, w_leg = np.polynomial.legendre.leggauss(NPTS_LEG)
    x_her, w_her = np.polynomial.hermite.hermgauss(NPTS_HER)

    antenna = pickle.load(open('real_antenna_s.p','rb'))
    d = (antenna['data'][len(antenna['data'])/2,:]+antenna['data'][:,len(antenna['data'])/2])/2.
    
    
    u=antenna['data'][len(antenna['data'])/2,:]
    v=antenna['data'][:,len(antenna['data'])/2]
    
    
    # Find peaks in dB scale
    y = 10*np.log10(d)
    x = antenna['angles']
